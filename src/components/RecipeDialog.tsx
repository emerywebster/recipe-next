...
const RecipeDialog = ({
  ...
  const [step, setStep] = React.useState<"url" | "details">(
    mode === "add" ? "url" : "details",
  );
  ...
  const handleUrlSubmit = async () => {
    ...
    try {
      ...
      if (!scrapedData.title && !scrapedData.imageUrl) {
      ...
      setFormData((prev) => ({
        ...
        ...scrapedData,
        description:
          scrapedData.description || `Recipe from ${scrapedData.source}`,
      }));
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-white max-w-[800px] max-h-[90vh] overflow-y-auto">
        ...
        <DialogHeader>
          <DialogTitle>
            {mode === "add" ? "Confirm Recipe Details" : "Edit Recipe"}
          </DialogTitle>
          <DialogDescription>
            {mode === "add"
              ? "Review and adjust the recipe details below"
